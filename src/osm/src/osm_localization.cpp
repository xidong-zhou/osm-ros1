...
namespace osm_planner {
	...
	if(flag)
	{
        ...
        positions_.cartesian_pose.orientation = tf::createQuaternionMsgFromYaw(yaw);

	ROS_INFO("id:%d",positions_.id);
	ROS_INFO("lat:%f,lon:%f,yaw:%f",lat,lon,yaw);
	ROS_INFO("x:%f,y:%f",positions_.cartesian_pose.position.x,positions_.cartesian_pose.position.y);
